...
-export([
	 ...
	 log_fun_for_server/4,
	 log_fun_example/4,
	 add_connects_in_init/11,
...
%% add a log function which just print error info
log_fun_example(Module, Line, Level, FormatFun) ->
	case Level of
...
log_fun_for_server(Module, Line, Level, FormatFun) ->
	case Level of	
		error->
			{Format, Arguments} = FormatFun(),
			common:messageBox(Format,Arguments);
%% 		debug-> 		
...
%% 			start_link(?GAMEDB_CONNECT_POOL,MySQLHostAddress,MySQLPort,MySQLUserName,MySQLPassword,MySQLDbName,fun log_fun_for_server/4,utf8);
%% 		true->
%% 			%% just for test and create a test connect to pool
%%     		start_link(p1, "192.168.1.10", undefined,"root", "bear", "test",fun log_fun_example/4,utf8)
%% 	end.
...
start1(PoolId, Host, Port, User, Password, Database, LogFun, Encoding,
    ...
    gen_server:StartFunc(
      {local, ?SERVER}, ?MODULE,
      [PoolId, Host, Port, User, Password, Database, LogFun, Encoding], [{timeout,50000}]).
...
get_result_reason(#mysql_result{error=Reason}) ->
    Reason;
%% add by wenziyong, sometimes there is no err code
get_result_reason(_) ->
    "unkown".
...
init([PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
	%% add by wenziyong, let the proccess can handle EXIT msg
	process_flag(trap_exit, true),
	
    LogFun1 = if LogFun == undefined -> fun log/4; true -> LogFun end,
	%% modified by wenziyong,start -> start_link
	%% not support LinkConnection param in this function,use start_link (hard code)
    case mysql_conn:start_link(Host, Port, User, Password, Database, LogFun1,
			  Encoding, PoolId) of
	{ok, ConnPid} ->
		%% add by wenziyong
	    ?Log2(LogFun1, debug,
		...
		%% modified by wenziyong
	    %{ok, add_conn(Conn, State)},
		NewState = add_conn(Conn, State),
		%% add by wenziyong, add another connections
		mysql_assist:add_another_connections(PoolId, Host, Port, User, Password, Database, Encoding,LogFun,NewState);
%% 		case ?IsJustForTest of
%% 		false->
%% 			%% add another two game db connects
%% 			case add_connects_in_init(PoolId, Host, Port, User, Password, Database, Encoding, true,LogFun,NewState,2) of
%% 			{ok, NewState1}->
%% 				%% add two log db connects
%% 				LogDbName  = loginServer:ini_ReadString( "GameServerSetup.txt", "LogDbName", "fxserverlog" ),
%% 				case add_connects_in_init(?LOGDB_CONNECT_POOL, Host, Port, User, Password, LogDbName, Encoding, true,LogFun,NewState1,2) of	
%% 					{ok, NewState2}->
%% 						NewState3 = mySqlProcess:register_prepared_stats(NewState2),
%% 						NewState4 = logdbProcess:register_prepared_stats(NewState3),
%% 						
%% 						{ok,NewState4};
%% 					Err->
%% 						Err
...
%% 			add_connects_in_init(PoolId, Host, Port, User, Password, Database, Encoding, true,LogFun,NewState,9)	
%% 		end;
	{error, _Reason} ->
	    ?Log(LogFun1, error,
...
%% add by wenziyong, add multiple connects in dispatch init function
add_connects_in_init(_PoolId, _Host, _Port, _User, _Password, _Database, _Encoding, _Reconnect,_LogFun,State,0) ->
	{ok,State};
add_connects_in_init(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,LogFun,State,ConnectNum) ->
	case add_connect_in_init(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,LogFun,State) of
		{ok, NewState}->
			add_connects_in_init(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,LogFun,NewState,ConnectNum-1);
		Err->
	    	Err
	end.
	

%% add by wenziyong, add another connect in dispatch init function
add_connect_in_init(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,LogFun,State) ->
    LogFun1 = if LogFun == undefined -> fun log/4; true -> LogFun end,
    case mysql_conn:start_link(Host, Port, User, Password, Database, LogFun1,
			  Encoding, PoolId) of
	{ok, ConnPid} ->
	    ...
	    Conn = new_conn(PoolId, ConnPid, Reconnect, Host, Port, User, Password,
			    Database, Encoding),
	    {ok, add_conn(Conn, State)};
	Err->
	    Err
...
%% add by wenziyong for register prepare in mysql
add_prepare({prepare, Name, Stmt}, State) ->
    LogFun = State#state.log_fun,
    Version1 =
	case gb_trees:lookup(Name, State#state.prepares) of
	    {value, {_Stmt, Version}} ->
    ...
    ?Log2(LogFun, debug,
	"add_prepare/2: ~p (ver ~p) ~p", [Name, Version1, Stmt]),
...
%% modified by wenziyong, add EXIT handle
handle_info(Msg, State) ->
	...
	{'EXIT',Pid, Why} ->
		LogFun = State#state.log_fun,
	    ?Log2(LogFun, error,
		  "received 'EXIT' signal from pid ~p , why:~p", [Pid,Why]),
	    {noreply, State}
	end.
...
reconnect_loop(Conn, LogFun, N) ->
	...
	{ok, ConnPid} ->
		%% modified by wenziyong, debug->error
	    ?Log2(LogFun, error,
