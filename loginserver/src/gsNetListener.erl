...
init([_Port, Module]) ->
	...
	GameServerPort = main:ini_ReadInt("LoginServer.txt","GSPort",44802 ),
    case gen_tcp:listen(GameServerPort, ?LISTEN_TCP_OPTIONS) of
	    {ok, Listen_socket} ->
	        %%Create first accepting process
	        {ok, Ref} = prim_inet:async_accept(Listen_socket, -1),
			?LOG_OUT( "---info: gs net listener succ." ),
	        {ok, #state{listener = Listen_socket,
	                    acceptor = Ref,
	                    module   = Module}};
	    {error, Reason} ->
			?ERROR_OUT( "---info: gs net listener fail:~p", Reason),
	        {stop, Reason}
    end.
...
handle_info({inet_async, ListSock, Ref, {ok, CliSocket}},
    ...
    try
		...
		{ok, Pid} = gsNetServerSup:start_child(CliSocket),
