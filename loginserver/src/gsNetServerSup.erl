-module(gsNetServerSup).
-author('wenziyong').
-behaviour(supervisor).
-export([start_child/1, start_link/2, init/1]).
-define(SERVER, ?MODULE).
-define(CLIENT_SUP, gsNetClientSup).
...
init([Port, Module]) ->
	GsNetListener = {gsNetServerSup,                         		% Id       = internal id
                  {gsNetListener, start_link, [Port, Module]},	% StartFun = {M, F, A}
                  permanent,                               		% Restart  = permanent | transient | temporary
                  2000,                                    		% Shutdown = brutal_kill | int() >= 0 | infinity
                  worker,                                  		% Type     = worker | supervisor
                  [gsNetListener]                           		% Modules  = [Module] | dynamic
              },
	
	GsNetClientSupervisor = {?CLIENT_SUP,
	...
	{ok,
        {{one_for_one, ?MAX_RESTART, ?MAX_TIME},
            [GsNetListener, GsNetClientSupervisor]
        }
    };
