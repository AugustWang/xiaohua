...
init([_Port, Module]) ->
	...
	ClientPort = main:ini_ReadInt("LoginServer.txt","ClientPort",44801 ),
    case gen_tcp:listen(ClientPort, ?LISTEN_TCP_OPTIONS) of
	    {ok, Listen_socket} ->
	        %%Create first accepting process
	        {ok, Ref} = prim_inet:async_accept(Listen_socket, -1),
			?LOG_OUT( "---info: net listener succ." ),
	        {ok, #state{listener = Listen_socket,
	                    acceptor = Ref,
	                    module   = Module}};
	    {error, Reason} ->
			?ERROR_OUT( "---info: net listener fail:~p", Reason),
	        {stop, Reason}
    end.
